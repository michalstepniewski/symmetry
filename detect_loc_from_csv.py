...
def main():
    ...
    import pandas as pd
    df = pd.read_csv('df_middle.csv')
    size = 10
    mask = (df['x']>size)&(df['y']>size)&(df['y']<image.shape[0]-size)&(df['x']<image.shape[1]-size)
    df = df[mask]
    sup_points = []
    ...
    #for point in tqdm(points[25:-25]):
      try:
        y, x = point
        min_img = image2[y-size:y+size, x-size:x+size]
        d = superm2(min_img)
        ...
        for point in points:
            y1, x1 = point
            image2[y1+y-size,x1+x-size] = 255
            sup_points.append([y1+y-size,x1+x-size])
